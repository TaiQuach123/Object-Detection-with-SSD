...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        #loc_data: [batch_size, 8732, 4]   (cx, cy, w, h)
        #conf_data: [batch_size, 8732, 21]
        #targets: [batch_size, num_obj, 5] (xmin, ymin, xmax, ymax, labels)

        num_batch = loc_data.size(0)
        num_dbox = loc_data.size(1)
        num_classes = conf_data.size(2)

        conf_t_label = torch.LongTensor(num_batch, num_dbox).to(self.device) #(num_batch, 8732) 
        loc_t= torch.Tensor(num_batch, num_dbox, 4).to(self.device) #(num_batch, 8732, 4)

        for idx in range(num_batch): # for each image
            #targets: (num_batch, num_objects, 5) (xmin, ymin, xmax, ymax, class)
            truths = targets[idx][:, :-1].to(self.device) #(num_objects, 4)
            labels = targets[idx][:,-1].to(self.device) #(num_objects,)
            
            dbox = dbox_list.to(self.device) #(8732, 4)    (cx_d, cy_d, w_d, h_d)
            variances = [0.1, 0.2]

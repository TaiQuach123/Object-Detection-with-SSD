...
def create_vgg():
    ...
    for cfg in cfgs:
        if cfg == 'M':
            ...
            conv2d = nn.Conv2d(in_channels=in_channels, out_channels=cfg, kernel_size=3, stride=1, padding=1)
            in_channels = cfg
...
def create_loc_conf(num_classes=21, bbox_ratio_num=[4,6,6,6,4,4]):
    loc_layers = []
    conf_layers = []
    in_channels_lst = [512, 1024, 512, 256, 256, 256]
    for i in  range (len(bbox_ratio_num)):
        loc_layers += [nn.Conv2d(in_channels_lst[i], bbox_ratio_num[i]*4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(in_channels_lst[i], bbox_ratio_num[i]*num_classes, kernel_size=3, padding=1)]
...
class SSD(nn.Module):
    def __init__(self, phase, cfg):
        ...
        #create main modules
        self.vgg = create_vgg()
        self.loc, self.conf = create_loc_conf()
    ...
    def forward(self, x):
        ...
        for source, location, confidence in zip(sources, self.loc, self.conf):
            #(batch_size, 4*aspect_ratio_num, height, width) --> (batch_size, height, width, 4*aspect_ratio_num)
            
            loc.append(location(source).permute(0, 2, 3, 1).contiguous()) #sau khi dung permute, cac phan tu dc sap xep ko lien tuc tren memory, neu dung view de thay doi shape ve sau thi se bi loi nen dung them contiguous
            conf.append(confidence(source).permute(0,2,3,1).contiguous())
...
def decode(loc, defbox_list):
    """
    parameters: 
    loc: [8732, 4] delta_cx, delta_cy, delta_w, delta_h
    defbox_list: [8732, 4] 
    ...
    boxes = torch.cat(
        (defbox_list[:, :2] + loc[:,:2]*0.1*defbox_list[:, 2:], 
...
def nms(boxes, scores, overlap=0.45, top_k=200):
    ...
    while idx.numel() > 0:
        ...
        tmp_h.resize_as_(tmp_x1)

        tmp_w = torch.clamp(tmp_x2 - tmp_x1, min=0.0)
        tmp_h = torch.clamp(tmp_y2 - tmp_y1, min = 0.0)

        inter = tmp_w * tmp_h
        other_area = torch.index_select(area, 0, idx)

        union = other_area + area[i] - inter
...
if __name__ == "__main__":
    ...
    #loc_layers, conf_layers = create_loc_conf(cfg['num_classes'], cfg['bbox_aspect_num'])
